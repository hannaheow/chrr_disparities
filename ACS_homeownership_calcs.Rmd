...
```{r}
## S: Subject tables (start with S)
v23_s <- load_variables(year = 2023, dataset = "acs5/subject", cache = TRUE)
...
```{r}
dp23 <- load_variables(2023, "acs5/profile", cache = TRUE)
...
# preserve tract-level rawvalues 

tract_level_all_years = lapply(names(acs_datasets), function(year) { 
    df = acs_datasets[[year]] %>% 
      rename(
      pop = B01001_001E,
      numerator = B25003_002E,
      denominator = B25003_001E,
...
tract_level_all_years = bind_rows(tract_level_all_years)
...
# Race groups: county level

-   8 groups

get v153 data by race: Tenure, 2023 ACS 5-Year Estimates, county level, tables: 
•	B25003B: Black
•	B25003C: AIAN
•	B25003D: ASIAN
•	B25003E: NHOPI
...
# to preserve rawvals at the race-group level 
race_level_all_years <- list(
  race_population_all_years,
  race_numerators_all_years,
  race_denominators_all_years
) %>%
...
processed_list <- imap(income_household_list, function(df, yr) {
  processed_df <- process_income_household(df, S1901_household) %>%
    mutate(year = as.integer(yr), .before = 1)
  return(processed_df)
})
...
v153_disparity_income_cis_all_years <- dplyr::left_join(
  ...
  summarise(statecode = first(statecode), 
            countycode = first(countycode),
...
# save raw value outputs(ie at the subgroup level) 
```{r}

# combine income_level_all_years with income_household_all_years in order to get all necessary cols 
income_ci_all_years$year = as.numeric(income_ci_all_years$year)
income_level_all_years = income_ci_all_years %>% left_join(income_household_all_years, by = c("statecode", "countycode", "grp", "year")) 

save(income_level_all_years, file = "calcd_data_how/income_level.RData")
save(race_level_all_years, file = "calcd_data_how/race_level.RData") 
save(tract_level_all_years, file = "calcd_data_how/tract_level.RData")
