...
income_level23 <- income_level_all_years %>%
  ...
  mutate(income_group = case_when(
    grp == 1 ~ 'Less than $5,000',
    grp == 1 ~ '$5,000 to $9,999',
    grp == 2 ~ '$10,000 to $14,999',
    grp == 3 ~ '$15,000 to $19,999',
...
# Get average correlation per state
top_bottom_states <- corr_by_state_plot %>%
  group_by(statecode) %>%
  summarise(avg_corr = mean(abs(Correlation), na.rm = TRUE)) %>%
  arrange(desc(avg_corr)) %>%
  slice(c(1:5, (n() - 4):n())) %>%
  pull(statecode)

# Filter to those 10 states
corr_by_state_plot_filtered <- corr_by_state_plot %>%
  filter(statecode %in% top_bottom_states)


library(ggplot2)

ggplot(corr_by_state_plot_filtered, aes(x = statecode, y = Correlation, color = strength)) +
  geom_point(size = 3, alpha = 0.8) +
  facet_grid(Var1 ~ Var2, drop = TRUE) +
  scale_color_manual(
    values = c("Strong" = "#E41A1C", "Moderate" = "#377EB8", "Weak" = "#4DAF4A"),
...
missing_summary <- data.frame(
  Domain = c("Race/Ethnicity", "Census Tract", "Household Income Categories"),
