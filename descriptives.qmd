...
The strongest correlation occurs between tract and income, weakest between tract and race. All correlations are significant.
...
NEED TO CHECK DENOMINATORS OF PERCENT MISSINGS
...
THE TABLES BELOW ARE A MESS - NEED TO CHECK DENOMINATORS OF THE PERCENT MISSINGS

```{r}
missing_by_rucc <- hist_data %>%
  mutate(
    RUCC_group = case_when(
      RUCC_2023 %in% c(1, 2, 3) ~ "Urban",
      RUCC_2023 %in% c(4, 5, 6, 7, 8, 9) ~ "Rural",
      TRUE ~ NA_character_  # Handle any unexpected RUCC codes
    )
  ) %>%
  group_by(RUCC_group) %>%
  summarise(
...
missing_by_rucc %>%
  gt() %>%
  tab_header(
    title = "Missing Data and Summary Statistics by Urban/Rural Category"
  ) %>%
  cols_label(
    RUCC_group = "Urban/Rural Category",
    race_missing = "Race Missing Count",
    race_percent_missing = "Race Percent Missing",
    income_missing = "Income Missing Count",
    income_percent_missing = "Income Percent Missing",
...
overall_row <- tibble(
  RUCC_group = "Overall",
  race_missing = sum(missing_by_rucc$race_missing),
  race_percent_missing = (sum(missing_by_rucc$race_missing) / nrow(hist_data)) * 100,
  race_summary = paste(
    round(median(hist_data$race_cis_sd_bgv_wt, na.rm = TRUE), 2),
    "(", round(min(hist_data$race_cis_sd_bgv_wt, na.rm = TRUE), 2),
    ",", round(max(hist_data$race_cis_sd_bgv_wt, na.rm = TRUE), 2),
    ")"
  ),
  
  income_missing = sum(missing_by_rucc$income_missing),
  income_percent_missing = (sum(missing_by_rucc$income_missing) / nrow(hist_data)) * 100,
  income_summary = paste(
    round(median(hist_data$income_cis_sd_bgv_wt, na.rm = TRUE), 2),
    "(", round(min(hist_data$income_cis_sd_bgv_wt, na.rm = TRUE), 2),
    ",", round(max(hist_data$income_cis_sd_bgv_wt, na.rm = TRUE), 2),
    ")"
  ),
  
  tract_missing = sum(missing_by_rucc$tract_missing),
  tract_percent_missing = (sum(missing_by_rucc$tract_missing) / nrow(hist_data)) * 100,
  tract_summary = paste(
    round(median(hist_data$tract_cis_sd_bgv_wt, na.rm = TRUE), 2),
    "(", round(min(hist_data$tract_cis_sd_bgv_wt, na.rm = TRUE), 2),
    ",", round(max(hist_data$tract_cis_sd_bgv_wt, na.rm = TRUE), 2),
    ")"
  )
)

# Combine the overall row with the rest of the data
missing_by_rucc <- bind_rows(missing_by_rucc, overall_row)
