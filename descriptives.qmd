...
nrace = race_level23 %>% group_by(statecode, countycode) %>% filter(pop!=0) %>% summarize(nrace = n_distinct(race))

noracedisp = nrace %>% filter(nrace <2) 

nincome = income_level23 %>% group_by(statecode, countycode) %>% filter(numerator!=0) %>% summarize(nincome = n_distinct(grp))

noincomedisp = nincome %>% filter(nincome <2) 
